...
def get_terms_for_quiz(user_id):
    terms = []
    with open("./data/terms.csv", "r", encoding="utf-8") as f:
        cnt = 1
        for line in f.readlines()[1:]:
            term, definition, source = line.split(";")
            if (str(source) == 'admin\n') or (str(source) == 'admin') or \
                    (str(source) == str(user_id)) or (str(source) == str(user_id) + '\n'):
                terms.append([cnt, term, definition])
            cnt += 1
    return terms


def write_term(new_term, new_definition, user_id):
    new_term_line = f"{new_term};{new_definition};{user_id}"
    with open("./data/terms.csv", "r", encoding="utf-8") as f:
